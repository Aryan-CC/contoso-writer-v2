...
# Agents
# Note: We no longer call the live researcher agent.
...
# Remove research field from Task (or ignore it) since we use cached research.
...
@trace
def create(product_context, assignment_context, evaluate=False):
    feedback = "No Feedback"

    yield building_agents_message()

    # Instead of calling the live researcher, load cached research data.
    yield start_message("researcher")
    base = Path(__file__).parent / "agents" / "writer"
    research_file = base / "research.json"
    if research_file.exists():
        research_result = json.loads(research_file.read_text())
    else:
        research_result = {"web": "No cached research data available."}
    yield complete_message("researcher", research_result)

    # Use a default research context.
    research_context = "Using cached research data."
    ...
    # Feedback loop: Use cached research data instead of regenerating research.
    retry_count = 0
    while str(editor_response["decision"]).lower().startswith("accept"):
        yield ("message", f"Sending editor feedback ({retry_count + 1})...")

        # Use cached research and pass along editor feedback only.
        editorFeedback = editor_response.get("editorFeedback", "No Feedback")

        yield start_message("writer")
        yield complete_message("writer", {"start": True})
        writer_result = writer.write(
            research_context, research_result, product_context, product_result, assignment_context, editorFeedback
        )
...
@trace  
def test_create_article(product_context, assignment_context):
    for result in create(product_context, assignment_context):
        parsed_result = json.loads(result)
        if isinstance(parsed_result, dict):
            if parsed_result.get('type') == 'researcher':
                print(parsed_result.get('data'))
            if parsed_result.get('type') == 'marketing':
                print(parsed_result.get('data'))
            if parsed_result.get('type') == 'editor':
                print(parsed_result.get('data'))
        elif isinstance(parsed_result, list) and parsed_result[0] == "writer":
...
if __name__ == "__main__":
    ...
    # Since we're using cached research, research_context is not needed.
    ...
    test_create_article(product_context, assignment_context)
