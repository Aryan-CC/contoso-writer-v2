...
def execute_research(instructions: str, feedback: str = "No feedback"):
    ...
    with project_client:
        ...
        @retry(
            ...
            wait=wait_exponential(multiplier=1, min=60, max=300),  # wait at least 60 seconds, up to 300 seconds
            stop=stop_after_attempt(4)  # only 4 attempts before giving up
        )
        def run_agent():
